...
export class Grid {
    ...
    initEvents() {
        for (const [position, gridItem] of this.gridItemArr.entries()) {
            ...
            gridItem.DOM.img.outer.addEventListener('mouseenter', () => {
                ...
                .to(gridItem.DOM.img.outer, { scale: .85 }, 'start')
                .to(gridItem.DOM.img.inner, { ease: 'power4', scale: 1.5 }, 'start')
            })
    ...
    calcTransformImage() {
        ...
        return {
            scale: winsize.height * .7 / imgrect.height,
            x: winsize.width * .5 - (imgrect.left + imgrect.width / 2),
            y: winsize.height * .5 - (imgrect.top + imgrect.height / 2),
        }
    }
